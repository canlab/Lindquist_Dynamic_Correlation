function [ timepoint, minBIC_1, minBIC_2] = Split1( Y, BIC, Delta, alpha, mask, Z )
...
if T < 2*Delta
    ...
    for t = Delta : T-Delta
        ...
        b1 = n1* ( log(det(S1new)) + trace(S1new\S1) );
        b2 = n2* ( log(det(S2new)) + trace(S2new\S2) );
    ...
    if minBIC_t == 0
        ...
        timepoint = minBIC_t;
        SignificantIndicator = SignificantSingle1( Y, timepoint, alpha, mask);
