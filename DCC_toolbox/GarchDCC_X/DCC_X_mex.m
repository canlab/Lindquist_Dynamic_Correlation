function [ H,R,Theta,X ] = DCC_X_mex( r, allpair, parallel )
%% (only difference with DCC_X: replace function LcOriginal with LcOriginal_mex)
...
if allpair==0
    x0 = rand(1,2)/2;
    X = fmincon(@(x)LcOriginal_mex(epsilon,x),x0,A2,B,[],[],LB2,[],[],options);
    if K==2
    ...
    if parallel == 0
        ...
        for i = 1:(K-1)
            for j = (i+1):K
                x0 = rand(1,2)/2;
                x = fmincon(@(x)LcOriginal_mex(epsilon(:,[i j]),x),x0,A2,B,[],[],LB2,[],[],options);  
                X(i,j,:) = x;
        ...
        parfor m = 1:M
                ...
                %j = Index(m,2);
                x0 = rand(1,2)/2;
                x = fmincon(@(x)LcOriginal_mex(epsilon(:,tempIndex),x),x0,A2,B,[],[],LB2,[],[],options);  
                XX(m,:) = x;
